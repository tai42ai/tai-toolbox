...
def test_pathological_jq_expression_surfaces_timeout_through_chain(bind_fake_app, monkeypatch):
    # A pathological/hostile jq expression must not block the loop indefinitely:
    # the off-loop helper's JQ_TIMEOUT_SECONDS budget surfaces as a loud
    # TimeoutError through execute_chain, and the second tool is never called.
    async def run_tool(key: str, arguments: dict[str, Any]) -> Any:
        if key == "source":
            return {"n": 1}
