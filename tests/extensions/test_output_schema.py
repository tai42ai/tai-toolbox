...
def test_factory_is_config_accepting_four_arg():
    # The skeleton apply site binds author config only to a factory that declares a
    # ``config`` parameter after ``func``/``name``/``description``; this is the
    # first config-accepting toolbox extension, so its signature must present it.
...
def test_constraint_keyword_is_enforced_at_runtime():
    # A value that satisfies the type but violates a constraint keyword
    # (``minLength``) is rejected at runtime by the faithful validator, with the
    # JSON-path of the offending node and the offending value carried on the error.
...
def test_composes_over_another_extensions_branch():
    # The post-hoc validate-and-advertise envelope composes over a branch produced
    # by another extension (a cache WRAPPER here): output_schema wraps the cache
    # branch, validates its result, and re-advertises the configured schema.
    calls: list[int] = []
