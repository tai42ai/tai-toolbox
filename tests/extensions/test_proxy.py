...
class _SlowHeaderProxy:
    """Dribbles partial header bytes slowly and never sends the header terminator,
    then holds the connection open. Each byte arrives faster than one connect
    timeout, so a per-recv timeout would never fire — only a whole-negotiation
...
class _HangingTlsHandshakeProxy:
    """Accepts the TCP connection for an HTTPS proxy but never runs the TLS
    handshake (it never answers the client's ClientHello), then holds the socket
    open. The client's ``wrap_socket`` blocks on the handshake, so only the connect
    deadline can bound the wait."""
...
def test_nested_route_restores_outer_on_inner_exit():
    # Nested routes must unwind by one level: leaving the inner block restores the
    # OUTER route (not None), so a proxied tool that invokes another proxied tool in
    # the same task keeps routing under the outer proxy once the inner call returns.
...
def test_http_connect_deadline_bounds_a_slow_trickle_proxy():
    # A proxy that dribbles header bytes forever, each faster than one connect
    # timeout, would reset a per-recv timeout indefinitely and wedge the connect.
    # The whole-negotiation deadline bounds the wait to ~connect_timeout, so it
    # raises promptly rather than after many trickle windows.
...
def test_https_proxy_connects_to_validated_ip_but_verifies_original_hostname(tmp_path, monkeypatch):
    # The subtle split: the TCP connection targets the validated IP (127.0.0.1),
    # while the TLS SNI and certificate verification use the ORIGINAL hostname. The
    # proxy presents a certificate for the hostname, not for 127.0.0.1 — so the
    # handshake succeeds only because SNI/verification use proxy_host, not the IP.
    ca = trustme.CA()
...
def test_https_proxy_connect_bounds_a_hanging_tls_handshake():
    # For an HTTPS proxy the TLS handshake runs under the same whole-negotiation
    # deadline as the TCP connect and the header recv. A proxy that accepts the TCP
    # connection but never runs the handshake would leave ``wrap_socket`` blocked
    # forever on a caller-blocking socket; the connect deadline bounds it instead, so
    # the handshake raises within ~connect_timeout rather than hanging.
...
def test_select_proxy_url_rotates_over_the_full_candidate_pool(monkeypatch):
    # Selection draws from the WHOLE candidate list, not a fixed index: the choice is
    # handed every pool entry so rotation can reach any of them. Pin that random.choice
    # sees the full pool (a fixed candidates[0] would pass it a one-element slice).
...
def test_connect_timeout_rejects_non_positive_values(monkeypatch, value):
    # A zero timeout puts the negotiation socket into non-blocking mode and a
    # negative one is nonsensical; both must fail loudly at construction rather
    # than silently break every proxied connect.
...
def test_registers_as_wrapper_named_proxy_requiring_body_locality(capture_registration):
    # ``requires_body_locality=True``: the routing wrapper only works in the
    # process running the tool body, so the bind engine must place it inside any
    # execution-relocating extension.
