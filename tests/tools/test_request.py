...
def test_request_reuses_a_keyed_session(local_server: Any) -> None:
    ...
    async def two_calls() -> None:
        # First call starts with an empty jar; the server's Set-Cookie lands in
        # the keyed session. The second call keeps the jar (no clear), so if the
        # session is genuinely reused it sends that cookie back.
