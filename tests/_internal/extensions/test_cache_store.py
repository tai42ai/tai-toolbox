...
def test_write_reclaims_an_expired_non_lru_entry_and_keeps_the_live_lru(
    ...
    # At cap, the new write reclaims the expired "b" rather than evicting the live
    # LRU "a": reclamation targets the expired entry regardless of LRU position, so
    # the live "a" survives even though it is the least-recently-used.
    store.write("c", 3, None)
...
def test_store_stays_over_cap_when_every_entry_is_locked() -> None:
    async def scenario() -> None:
        ...
        # Two concurrent single-flight callers each hold their key's lock while
        # writing; neither entry may be evicted, so the store exceeds its cap
