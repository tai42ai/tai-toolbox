...
The guard serializes concurrent same-key requests (so a pooled curl session's
shared RESOLVE pin is never clobbered mid-flight) while evicting each key's entry
the moment no holder or waiter references it, so the inner lock map cannot grow
for the process lifetime on distinct (LLM-supplied) session keys.
"""
...
def test_distinct_key_sequential_guards_leave_map_empty() -> None:
    # Each distinct session_key gets its own entry while its guard is held, and
    # the entry is evicted on release, so after N sequential guards the current
    # loop's inner map is empty — no per-key growth.
...
def test_waiter_reuses_the_same_lock_object() -> None:
    # A waiter arriving while a holder is inside references the SAME entry (its
    # refcount rises to 2) and acquires the SAME lock object.
    async def scenario() -> None:
