...
def test_http_tool_reports_streamed_body_metrics(monkeypatch: pytest.MonkeyPatch, local_server: Any) -> None:
    """The streaming path reports full-body transfer metrics, not curl's stale
    time-to-first-byte values: download_size is the body length and elapsed is a
...
def test_http_tool_pins_each_host_in_a_pooled_session(monkeypatch: pytest.MonkeyPatch, local_server: Any) -> None:
    """A pooled (keyed) session reused across two different hosts pins each
    request to its own validated address. curl's ``--resolve`` mapping is set per
    request, so the second host is pinned too — were it set only at session
    creation, the second host would miss its mapping and fail DNS resolution."""
    _enable(monkeypatch, allow_cidrs=["127.0.0.0/8"])
...
def test_http_tool_rejects_a_caller_proxy_at_an_internal_address(
    ...
    # A caller-supplied proxy is unguarded by curl's native path: even with the
    # destination pinned, the request would reach it through an internal proxy. The
    # guard validates each proxy host and rejects an internal one before any request.
    _enable(monkeypatch, allow_cidrs=["127.0.0.0/8"])
    with pytest.raises(UrlGuardError):
        asyncio.run(
            request(
...
def test_http_tool_pins_a_caller_proxy_to_its_validated_ip(monkeypatch: pytest.MonkeyPatch) -> None:
    """A caller-supplied proxy host that passes the guard (resolves to a public
    address) must be PINNED to that validated address in curl's RESOLVE map, not
    left for curl to re-resolve at connect — otherwise the host could answer public
    at validation and internal at connect (DNS-rebinding on the proxy hop). The map
    curl receives must carry a proxy pin ``host:port:validated_ip`` alongside the
    destination pin."""
...
def test_http_tool_serializes_concurrent_same_key_pins(monkeypatch: pytest.MonkeyPatch, local_server: Any) -> None:
    """Two concurrent requests sharing a ``session_key`` each pin a different,
    non-resolvable hostname to the loopback server. The pooled ``CurlClient`` hands
    both the same ``AsyncSession`` with a shared RESOLVE mapping, so without the
    per-key lock the second call would clobber the first's pin mid-flight and the
    ...
    # Capture the per-key lock the request path uses and record whether it is held
    # while each request streams its body — ``enforce_size`` runs inside the locked
    # critical section, so a recorded ``True`` proves the pin is set and consumed
    # under the exclusive lock, never racing a concurrent same-key request.
