...
The provider factory and settings live in tai-kit behind its ``llm`` extra;
this tool's backing dependency is opt-in as ``pip install
'tai-toolbox[embeddings]'``. Missing it raises a loud install hint at import
...
try:
    ...
    raise ImportError(
        "tai-toolbox 'embeddings' tools require the 'embeddings' optional "
        "dependency (tai-kit[llm]). "
        "Install it with: pip install 'tai-toolbox[embeddings]'"
    ) from exc

from tai_contract.app import tai_app
from tai_kit.llm.settings import embedding_settings, llm_provider_settings
