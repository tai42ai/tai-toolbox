"""The ``request`` tool: an HTTP request backed by tai-kit's pooled curl client.

Requests without a ``session_key`` use a one-shot session; a ``session_key``
reuses a pooled session (shared cookies and connections) across calls. The
request execution, response serialization, and SSRF pinning live in
:mod:`tai_toolbox._internal.tools.http_client`; this entrypoint is thin.

The curl client lives in tai-kit behind its ``curl`` extra; this tool's backing
dependency is opt-in as ``pip install 'tai-toolbox[http]'``. Missing it raises a
loud install hint at import time rather than a silent skip.
"""
...
from tai_toolbox._internal.tools.http_client import perform_request
