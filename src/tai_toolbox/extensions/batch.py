...
Branches a tool into a ``<tool>_batch`` variant that runs many instances of the
wrapped tool from a list of input parameter sets, sequentially or in parallel, and
returns their results in input order. The variant presents its OWN composed
signature (built with :func:`makefun.create_function`): a ``params`` list typed by
a generated per-tool input model, plus ``execution_mode``, ``max_concurrent``, and
``fail_fast`` controls. The batch runner lives in
:mod:`tai_toolbox._internal.extensions.batch_executor`.
"""
...
from tai_toolbox._internal.extensions.batch_executor import execute_batch


@tai_app.extensions.extension(kind=ExtensionKind.TRANSFORMER, name="batch")
def batch(func, orig_name, orig_desc):
    ...
    # Build a per-tool pydantic input model from the wrapped tool's parameters so
    # each batch item is validated with the tool's own schema.
    ...
    # Expose the generated model in the tool's module namespace so its schema
    # ``$ref`` resolves by qualified name during exposure-schema generation.
