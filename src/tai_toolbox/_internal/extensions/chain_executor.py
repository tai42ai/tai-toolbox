...
Needs the ``chain`` extra (jq, via ``tai-kit[jq]``); the module fails loudly at
...
try:
    ...
    raise ImportError(
        "The 'chain' tool extension needs the jq library. Install it with: pip install 'tai-toolbox[chain]'"
    ) from exc
...
async def execute_chain(
    ...
    first_result = await tai_app.tools.run_tool(tool_name, tool_arguments)
    jq_result = await run_jq_first(jq_expression, first_result)
    return await tai_app.tools.run_tool(next_tool_name, jq_result)
