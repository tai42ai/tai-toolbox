"""Request execution for the ``request`` tool, backed by tai42-kit's pooled curl
client.
...
streams the body under the size cap. The curl client lives in tai42-kit behind its
``curl`` extra; this module's backing dependency is opt-in as
``pip install 'tai42-toolbox[http]'``. Missing it raises a loud install hint at
