...
:mod:`tai42_toolbox._internal.extensions.chain_executor`, which needs the ``chain`` extra
(jq, via ``tai42-kit[jq]``) and fails loudly at import with an install hint when jq
